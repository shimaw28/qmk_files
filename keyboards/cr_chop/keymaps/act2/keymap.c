...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_DVORAK] = LAYOUT( \
     ...
     KC_LSFT,                SW_SCLN_CLN, KC_Q,    KC_J,   KC_K,    KC_X,                                                 KC_B,           KC_M,   KC_W, KC_V, KC_Z,         KC_ENT , \
  ...
  [_ADJUST] = LAYOUT( \
    RESET,   TO(_QWERTY), MAGIC_SWAP_ALT_GUI,   KC_NO,   KC_NO, KC_NO,                                KC_NO, KC_NO,   KC_NO, KC_NO, KC_NO, KC_NO,\
    RGB_HUI, TO(_DVORAK), TO(_DVORAK_JP),   RGB_VAI,    KC_NO,           KC_NO,                   KC_NO, KC_NO,   KC_NO, KC_NO, KC_NO, KC_NO,\
    SW_EJ,   TO(_DVORAK_WIN), TO(_DVORAK_JP_WIN),       RGB_VAD,    KC_NO,           KC_NO,                   KC_NO, KC_NO,   KC_NO, KC_NO, KC_NO, KC_NO,\
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  switch (keycode) {
    case DJ_QUOT:
     if (dj_mode == 0){
        if (record->event.pressed) {SEND_STRING("'"); dj_mode = 0;}
     } else {
        if (record->event.pressed) {SEND_STRING("ai"); dj_mode = 0;}
      }
      return false;
    ...
    case DJ_COMM:
     if (dj_mode == 0){
        ...
        if (record->event.pressed) {SEND_STRING("ou"); dj_mode = 0;}
      }
      return false;
    ...
    case DJ_DOT:
     if (dj_mode == 0){
        ...
        if (record->event.pressed) {SEND_STRING("ei"); dj_mode = 0;}
      }
      return false;
    case DJ_P:
        if (record->event.pressed) {SEND_STRING("p"); dj_mode = 1;}
        return false;

    case DJ_Y:
        if (record->event.pressed) {SEND_STRING("y"); dj_mode = 1;}
        return false;

    case DJ_A:
        if (record->event.pressed) {SEND_STRING("a"); dj_mode = 0;}
        return false;

    case DJ_O:
        if (record->event.pressed) {SEND_STRING("o"); dj_mode = 0;}
        return false;

    case DJ_E:
        if (record->event.pressed) {SEND_STRING("e"); dj_mode = 0;}
        return false;

    case DJ_U:
        if (record->event.pressed) {SEND_STRING("u"); dj_mode = 0;}
        return false;

    case DJ_I:
        if (record->event.pressed) {SEND_STRING("i"); dj_mode = 0;}
        return false;
    ...
    case DJ_SCLN:
      if (dj_mode == 0){
        ...
        if (record->event.pressed) {SEND_STRING("ann"); dj_mode = 0;}
      }
      return false;

    case DJ_Q:
     if (dj_mode == 0){
        if (record->event.pressed) {SEND_STRING("q"); dj_mode = 0;}
     } else {
        if (record->event.pressed) {SEND_STRING("onn"); dj_mode = 0;}
      }
      return false;

    case DJ_J:
      if (dj_mode == 0){
        if (record->event.pressed) {SEND_STRING("j"); dj_mode = 0;}
      } else {
        if (record->event.pressed) {SEND_STRING("enn"); dj_mode = 0;}
      }
      return false;

    case DJ_C:
      if (dj_mode == 0){
        if (record->event.pressed) {SEND_STRING("c"); dj_mode = 0;}
      } else {
        if (record->event.pressed) {SEND_STRING("unn"); dj_mode = 0;}
      }
      return false;

    case DJ_X:
      if (dj_mode == 0){
        if (record->event.pressed) {SEND_STRING("x"); dj_mode = 0;}
      } else {
        if (record->event.pressed) {SEND_STRING("inn"); dj_mode = 0;}
      }
      return false;
    ...
    case DJ_F:
        if (record->event.pressed) {SEND_STRING("f"); dj_mode = 1;}
        return false;

    case DJ_G:
        if (record->event.pressed) {SEND_STRING("g"); dj_mode = 1;}
        return false;

    case DJ_K:
        if (record->event.pressed) {SEND_STRING("k"); dj_mode = 1;}
        return false;

    case DJ_R:
        if (record->event.pressed) {SEND_STRING("r"); dj_mode = 2;}
        return false;

    case DJ_L:
        if (record->event.pressed) {SEND_STRING("l"); dj_mode = 2;}
        return false;

    case DJ_D:
        if (record->event.pressed) {SEND_STRING("d"); dj_mode = 1;}
        return false;
    ...
    case DJ_H:
        if (record->event.pressed) {
            if (dj_mode == 2) {
                ...
                SEND_STRING("h"); dj_mode = 1;
            }
        }
        return false;

    case DJ_T:
        if (record->event.pressed) {SEND_STRING("t"); dj_mode = 1;}
        return false;
    ...
    case DJ_N:
        if (record->event.pressed)  {
            if (dj_mode == 1) {
                ...
                SEND_STRING("n"); dj_mode = 2;
            }
        }
        return false;

    case DJ_S:
        if (record->event.pressed) {SEND_STRING("s"); dj_mode = 2;}
        return false;

    case DJ_B:
        if (record->event.pressed) {SEND_STRING("b"); dj_mode = 1;}
        return false;

    case DJ_M:
        if (record->event.pressed) {SEND_STRING("m"); dj_mode = 1;}
        return false;

    case DJ_W:
        if (record->event.pressed) {SEND_STRING("w"); dj_mode = 1;}
        return false;

    case DJ_V:
        if (record->event.pressed) {SEND_STRING("v"); dj_mode = 2;}
        return false;

    case DJ_Z:
        if (record->event.pressed) {SEND_STRING("z"); dj_mode = 2;}
        return false;
// -------------------------------------
    case DJ_XTU:
        if (record->event.pressed) {SEND_STRING("xtu"); dj_mode = 0;}
        return false;

    case DJ_CLEAR:
        if (record->event.pressed) {dj_mode = 0; }
        return false;
